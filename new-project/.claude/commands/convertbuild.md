...
11. **`cloneNode(true)` before `classList.add` on the original.** `cloneNode` copies every class that exists on the element at clone time. If you call `classList.add('hide-original')` first, the clone inherits that class and your CSS will hide both. Always clone first, then mutate the original.
12. **Third-party widgets (Payflex, Yotpo, etc.) render async inside their host element.** The host (`.product-form`, `.buy-me-box`) exists in the DOM long before the widget renders inside it. Always gate any operation on the widget with its own `waitForElement('.payflex-product-widget', doThing)` — never assume the widget is present when the parent loads.
...
13. **Vue/SPA re-renders wipe injected CSS classes** — Vue replaces component DOM nodes after async data loads (price, stock, options). Classes added via JS to those nodes are silently lost. `trigger()` re-polling for 7s is not enough if the re-render happens after the window closes. Attach a `MutationObserver` on the component root (`{ childList: true, subtree: true }`) and re-apply classes in the callback, debounced ~100ms. `classList.add` is idempotent — safe to call on every mutation without a guard.
14. **Closure-level injection flags survive SPA navigation but not Vue re-renders** — A flag like `var injected = { section: true }` stays `true` for the lifetime of the JS context even after the DOM is fully replaced. Use closure flags only for one-shot async work (API calls, event listeners) where re-running would cause real harm. For DOM class applications and structural wrappers, use a DOM presence check (`document.querySelector('.cro-xxx-wrapper')`) as the guard — it automatically resets when Vue replaces the component tree.
15. **Set injection flags only after all required elements are confirmed** — Setting `injected.thing = true` at the top of a function before checking whether all target selectors exist permanently locks the function out if any element loads late on the first call. Collect and confirm all required elements first; only set the flag immediately before the mutation.
16. **Inline `display:none` + CSS `!important` for SPA body-class removal** — On SPAs, Convert removes the variation body class when navigating to non-target pages, but injected HTML stays in the DOM and becomes visible at the browser's default display value. Pattern: set `style="display:none"` inline on every injected root element, then override in CSS with `body.<variation_class> .your-element { display: <correct-value> !important; }`. The `!important` wins when the body class is present; the inline style hides the element when the body class is absent.
17. **Yotpo (and similar review widgets) load reviews on scroll, not on page load** — `waitForElement('.yotpo-review-card', ...)` will time out at 15s on most PDPs because the reviews section is below the fold and lazy-loads on scroll. Use the Yotpo v3 REST API directly: `https://api-cdn.yotpo.com/v3/storefront/store/<key>/product/<sku>/reviews`. The product SKU is available in the buy box form as `input[name="sku"]`. This is faster, scroll-independent, and avoids injecting a second widget instance into the DOM. Reset the injection flag in `.catch()` and on early exits so a network failure allows a retry.
