           ...
           // oReq.open("POST", "https://poptopdesk.com/blockeddeal/Emailforblocked/insertCatalogue", true);
            oReq.onload = function (oEvent) {
                ...
                if (oReq.status == 200) {
                   ...
                   if(bs_data.personemail != 'null' && bs_data.personemail != null ){
                        const catalogueEmail = bs_data.personemail;
                        console.log(`Email : ${catalogueEmail}`);
                        window.dataLayer = window.dataLayer || [];
                        window.dataLayer.push({
                            'event': 'catalogue_request_form_submitted', // catalogue_request_form_submitted rename to catalogue_request in gtm and added param catalogue_name value catalogueName on 2/10/2026
                                ...
                                'GA4_eventLabel':catalogueName,
                                'GA4_eventValue':'Blocked sequence',
...
function ga4ButtonTrack(){
    let buttons = document.querySelectorAll('.bs-ga4-button-track');
    buttons.forEach((button,i) => {
        button.addEventListener('click', function(event) { 
                        let option =  buttons[i].getAttribute('data-ga4-param-option');
                        //console.log(option);
                        window.dataLayer = window.dataLayer || [];
                        window.dataLayer.push({
                            'event': 'bs_click_option',
                            'GA4_eventValue':option, 
                            'GA4_eventAction':'Button clicked',
                            'GA4_eventLabel':'Email'
                        });
        })
    }) 
